...
define(function (require) {
	var init = function () {
		...
		var slidesNodeList = slider.querySelectorAll('.wrapper .slide'); //Convert this nodeList to a real array
		var slides = [];
		...
		var thisAnimationEvents = require('./animationEvents');
		...
		function doFadeIn() {
			...
			setMouseEvents(); //Don't set mouse events until it's loaded, otherwise we might get errors
			...
			if (slidesLoaded !== slideCount) {
				...
				slides[index].addEventListener('load', function svgLoad() {
					...
					if (numberOfLoadedSlides() === slideCount) {
						...
						slidesLoaded = slideCount; //Only do this once;
