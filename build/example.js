  ...
  var init = function () {
    ...
    function doRun(obj) {
      ...
      timeout = setTimeout(function () {
        ...
        if (!animationEvents.isSupported()) {
          doRun(next);
        }
      }, 5000);
    }
    ...
    function setMouseEvents() {
      slider.addEventListener('mouseover', function hoverOn() {
        ...
        animationEvents.handler(slides, false, doRun);
      });
      slider.addEventListener('mouseout', function hoverOff(event) {
        ...
        animationEvents.handler(slides, true, doRun);
        if (isAnimating) {
    ...
    function doFadeIn() {
      ...
      animationEvents.fadeIn(slides[0]);
      animationEvents.fadeIn(ticker);
      if (!animationEvents.isSupported()) {
        doRun(slides[0]);
